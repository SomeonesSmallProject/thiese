...
#        x = self.conv1(x, edge_index)
    ...
    def simple_con2(self, graph):
        x, edge_index, edge_weight = graph.x, graph.edge_index, graph.edge_attr
        x = self.conv3(x, edge_index, edge_weight)
        x = self.conv1(x, edge_index, edge_weight)
        tmp_Graph = Data(x=x, edge_index=edge_index)
        return tmp_Graph
    ...
    def CNN1(self, graph):
        ...
        edge_index = edge_index.type(torch.long)
        tmp_Graph = self.simple_con1(
            Data(x=x, edge_index=edge_index, edge_weight=edge_weight)
        )
        x = tmp_Graph.x
...
#        Feature_relu = F.elu(x)
        ...
        Feature_cov2 = self.conv2(x=Feature_relu, edge_index=edge_index)
        tmp_Graph = Data(x=Feature_cov2, edge_index=edge_index,edge_attr=edge_weight)
        return tmp_Graph
...
#        Feature_relu = F.elu(x)
    ...
    def forward(self, graph):
        ...
        Cnn_graph = self.CNN1(graph)
        Cnn_graph2 = self.CNN2(Cnn_graph)
        Cnn_graph2_elu = Data(x=F.elu(Cnn_graph2.x),
                              edge_index=Cnn_graph.edge_index)
        Cnn_graph3 = self.CNN2(Cnn_graph2_elu)
        Cnn_graph4 = self.CNN2(Cnn_graph2_elu)
        tmp_graph = Data(
            x=sum(sum(Cnn_graph.x, Cnn_graph2_elu.x),
                  sum(Cnn_graph3.x, Cnn_graph4.x)),
...
class Modules(nn.Module):
    def __init__(
        ...
        Protein_InputChannel = super_dict["Protein_InputChannel"]
        p = super_dict["p"]
        pad_size = super_dict["pad_size"]
        device = super_dict["device"]
        mean = super_dict["mean"]
        ...
        self.Protein = Protein(
            input_channel=Protein_InputChannel, output_channel=output_channel)
        self.MultHeaderAttention = Transformer(
            input_dim=21,
            hidden_dim=64,
            num_layers=16,
            num_heads=32,
        ...
        self.protein_bias = torch.randn(size=[output_channel,output_channel], requires_grad=True,device=device)
        #self.sequence_bias = torch.randn(size=[output_channel,output_channel], requires_grad=True,device=device)
        ...
        self.output_pad_size_1 = pad_size[1]

        self.paded_liner1 = nn.Linear(pad_size[0], output_channel)
        self.paded_liner2 = nn.Linear(pad_size[1], output_channel)
        ...
        #self.conv_out_size = (
        #    output_channel-4*(kernel_size[0])+4, output_channel-4*(kernel_size[1])+4)

        
        self.max = 1

        self.liner = nn.Linear(
            in_features=self.conv_out_size[0], out_features=self.max
        )
    ...
    def forward(self, data):
        Protein_graph = data[0]
        Molucula_graph = data[1]
        sequence = data[2]

        Protein_Output = self.Protein.forward(Protein_graph)
        Molucula_Output = self.Molucula.forward(Molucula_graph)
        sequence_output = self.MultHeaderAttention.forward(sequence)

        Protein_Output_adj = to_dense_adj(
            Protein_Output.edge_index.type(torch.long))[0]

        protein_softmax = 10*(F.softmax(Protein_Output.x)-0.5)

        tmp = protein_softmax * sequence_output
        #tmp = Protein_Output.x * sequence_output
        tmp_size = [tmp.shape[-2], tmp.shape[-1]]

        tmp = torch.reshape(tmp, (tmp_size[0], tmp_size[1]))

        #Protein_Output_Add = torch.mm(Protein_Output_adj, tmp)
        Protein_Output_Add = self.dropout(tmp)
        #Protein_Output_Add = self.dropout(Protein_Output_Add)

        Molucula_Output_adj = to_dense_adj(Molucula_Output.edge_index.type(torch.long))[
            0
        ]
        #Molucula_Output_Add = torch.mm(
        #    Molucula_Output_adj, F.softmax(Molucula_Output.x)-0.5)
        #Molucula_Output_Add = torch.mm(
        #    Molucula_Output_adj, Molucula_Output.x)
        Molucula_Output_Add = Molucula_Output.x
        Molucula_Output_Add = self.dropout(Molucula_Output_Add)
        Output_x = torch.mm(Protein_Output_Add, Molucula_Output_Add.T)

        Output_x = F.elu(Output_x)

        left_pad = int(0.5 * (self.output_pad_size_1 - Output_x.size(1)))
        right_pad = int(self.output_pad_size_1 - Output_x.size(1) - left_pad)
        top_pad = int(0.5 * (self.output_pad_size_0 - Output_x.size(0)))
        down_pad = int(self.output_pad_size_0 - Output_x.size(0) - top_pad)
        Output_x = F.pad(Output_x, (left_pad, right_pad, top_pad, down_pad))

        tmp_output = self.paded_liner2(Output_x)
        ...
        output = sum(output, self.mean)
        return output


    def forward_no_softmax(self, data):
        ...
        protein_biased = torch.mm(Protein_Output.x , self.protein_bias)

        #sequence_biased = torch.mm(sequence_output , self.sequence_bias)
        tmp = protein_biased * sequence_output
        tmp_size = [tmp.shape[-2], tmp.shape[-1]]
...
#        Molucula_biased = torch.mm(Molucula_Output.x , self.Chem_bias)
#        Molucula_Output_Add = torch.mm(
#            Molucula_Output_adj, Molucula_biased)
        Molucula_Output_Add = torch.mm(
            Molucula_Output_adj, Molucula_Output.x)
        Molucula_Output_Add = self.dropout(Molucula_Output_Add)
        Output_x = torch.mm(Protein_Output_Add, Molucula_Output_Add.T)
